...
def image_put(queue,camera_ip):
    ...
    while is_opened:
        is_opened, frame = cap.read()
        queue.put(frame)
        queue.get() if queue.qsize() > 24 else None
    cv2.VideoCapture(camera_ip).release()
def show_result(AI_queue):
...
def video_feed(AI_queue):
    # Video streaming route. Put this in the src attribute of an img tag
    return Response(show_result(AI_queue), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def run_multi_camera(rtsp='rtmpCollection.txt'):
    ...
    AI_result_queue=[mp.Queue(maxsize=1) for _ in camera_ip_l]
    processes = []
    global AI_queue
    for input_queue,AI_queue, camera_ip in zip(frame_queues,AI_result_queue,camera_ip_l):
        processes.append(mp.Process(target=image_put, args=(input_queue,camera_ip)))
        processes.append(mp.Process(target=AI_process, args=(input_queue,AI_queue)))
